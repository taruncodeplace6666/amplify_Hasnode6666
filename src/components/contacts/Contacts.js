...
function Contacts() {
  ...
  const getContacts = async () => {
    try {
      ...
      const profilePicPromises = contactsList.map(async (contact) => {
        const contactProfilePicPath = contact.profilePicPath;
        try {
          const contactProfilePicPathURI = await Storage.get(contactProfilePicPath, {
            expires: 60,
          });
      ...
      const profilePicPathsResult = await Promise.all(profilePicPromises);
      setProfilePicPaths(profilePicPathsResult);
    } catch (err) {
  ...
  const addNewContact = async () => {
    try {
      ...
      // Update profilePicPaths state with the new profile pic path
      setProfilePicPaths([...profilePicPaths, URL.createObjectURL(profilePic)]);
      // Clear the contactData and profilePic state after successful addition
      setContactData({ name: '', email: '', cell: '' });
      setProfilePic('');
    } catch (err) {
        ...
        <Col sm={3}>
          ...
          <Form>
            ...
            <Form.Group className="mb-3" controlId="formBasicText">
              ...
              <Form.Control type="file" accept="image/png" onChange={(evt) => setProfilePic(evt.target.files[0])} />
            </Form.Group>
